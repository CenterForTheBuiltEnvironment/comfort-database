  ...
  tabsetPanel(
    # position="left", # the "position" function is deprecated"
              ...
              tabPanel(id=paste0("bldg",id),title="building",
                       ...
                       # wellPanel(
                       #   checkboxGroupInput(paste0("type",id), "Conditioning type:",
                       #                      list("Natural" = "NV", 
                       #                           "Mechanical " = "HVAC", 
                       #                           "Mixed mode" = "MM",
                       #                           "Undefined" = "Undefined"),
...
#                        wellPanel(
#                          checkboxInput(paste0("pc",id),"Filter by personal controls"),
#                          conditionalPanel(condition=paste0("input.pc",id),
#                                           radioButtons(paste0("andor",id),"",choices=list("Has all of" = "and", "Has at least one of" = "or","Does not have" = "no"),selected="or"),
#                                           uiOutput(paste0("controls",id)))),
                       wellPanel(
                         checkboxInput(paste0("winfilter",id),"Filter by window control"),
                         conditionalPanel(condition=paste0("input.winfilter",id),
                                          checkboxGroupInput(paste0("win",id),label="",choices=list("Available" = "TRUE","Not available" = "FALSE"),selected=c("TRUE","FALSE"))),
                         checkboxInput(paste0("extfilter",id),"Filter by external door control"),
                         conditionalPanel(condition=paste0("input.extfilter",id),
                                          checkboxGroupInput(paste0("ext",id),label="",choices=list("Available" = "TRUE","Not available" = "FALSE"),selected=c("TRUE","FALSE"))),
                         checkboxInput(paste0("intfilter",id),"Filter by internal door control"),
                         conditionalPanel(condition=paste0("input.intfilter",id),
                                          checkboxGroupInput(paste0("int",id),label="",choices=list("Available" = "TRUE","Not available" = "FALSE"),selected=c("TRUE","FALSE"))),
                         checkboxInput(paste0("thermofilter",id),"Filter by thermostat control"),
                         conditionalPanel(condition=paste0("input.thermofilter",id),
                                          checkboxGroupInput(paste0("thermo",id),label="",choices=list("Available" = "TRUE","Not available" = "FALSE"),selected=c("TRUE","FALSE"))),
                         checkboxInput(paste0("curfilter",id),"Filter by curtain or blind control"),
                         conditionalPanel(condition=paste0("input.curfilter",id),
                                          checkboxGroupInput(paste0("cur",id),label="",choices=list("Available" = "TRUE","Not available" = "FALSE"),selected=c("TRUE","FALSE"))),
                         checkboxInput(paste0("heatfilter",id),"Filter by heater control"),
                         conditionalPanel(condition=paste0("input.heatfilter",id),
                                          checkboxGroupInput(paste0("heat",id),label="",choices=list("Available" = "TRUE","Not available" = "FALSE"),selected=c("TRUE","FALSE"))),
                         checkboxInput(paste0("fanfilter",id),"Filter by fan control"),
                         conditionalPanel(condition=paste0("input.fanfilter",id),
                                          checkboxGroupInput(paste0("fan",id),label="",choices=list("Available" = "TRUE","Not available" = "FALSE"),selected=c("TRUE","FALSE")))
                       ),
...
# HTML('<hr style="border-color: black;">'),
               ...
               fluidRow(                 
                 ...
                 column(4,
                        probitSample("1","rgba(124,160,255,.5)","Database 1")
                 ),
                 
                 column(4,
                        probitSample("2","rgba(178,125,113,.5)","Database 2")
                 ),
             ...
             tabPanel("Adaptive model",fluidPage(
                 ...
                 column(3,
                        selectInput(inputId="satmet", label="Satisfaction metric:",
                                    choices=list(#"All data" = "ASH",
                                                 "Acceptability" = "TSA",
                                                 #"Calculated acceptability" = "TSA_ord",
                                                 #"Satisfaction calculated from probit sample set 1" = "calc",
                                                 #"Thermal sensation +-1.5" = "acc15", 
                                                 "Thermal sensation +-2" = "acc2", 
               ...
               fluidRow(
                 column(4,
                        br(),
                        filterOptions("_bins1","Database 1",F,F)
                 ),
                 column(2,
                        br()
                 ),
                 column(4,
                        ...
                        conditionalPanel(condition="input.compare_bins",
                                         filterOptions("_bins2","Database 1",F,F))                     
                 ),
                 column(2)
               )
             )
               ...
               fluidRow(
                 ...
                 column(3,
                        selectInput("xaxis_elev", "Selected x-axis temperature:",
                                    list("Operative" = "TOP",
                                         "Air" = "TAAV",
                                         "Prevailing mean outdoor" = "prev_ta",
                                         #"Mean daily outdoor" = "dayav_ta",
                                         "Radiant" = "TRAV"),selectize=F),
             ...
             tabPanel("About",fluidPage(
               fluidRow(
                 ...
                 p("The data shown in this tool is a compilation of many individual thermal comfort field studies in which building occupants were surveyed about their thermal comfort while simultaneous physical measurements were taken.
                   Because the data comes from multiple independent studies, not all of the information was collected for each observation.
                   The data includes ASHRAE Comfort Database I and Database II. The database is a public-domain resource."),
                 ...
                 p("Recognizing the value of open-source research databases, in 2014, project of the ASHRAE Global Thermal Comfort Database II was launched that took on the challenge of systematically collecting and harmonizing the raw data collected in comfort field studies since 1997 (when the ASHRAE database I was built) from around the world into an expanded, quality-assured database.
                   More than thirty-five thermal comfort research groups from around the world generously made their raw data freely available to the project and in total contributed with more than 70,000 rows of paired subjective comfort votes and objective instrumental measurements of thermal comfort parameters."),
                 br(),
                 ...
                 div(HTML("<ol>
                              ...
                              <li>Building level data</li>
                                    <ul>
                                        ...
                                        <li>Climate:            Season (Spring, Summer, Autumn, Winter), Climatic zone (N>10)</li>
                                        <li>Conditioning types: Air-conditioned (AC), Naturally ventilated (NV), Mixed mode (MM), Mechanical ventilated (MV)</li>
                              ...
                              <li>Survey information</li>
                                    <ul>
                                        ...
                                        <li>Thermal acceptability: Right now, is the temperature at your workplace acceptable? (yes or no)</li>
                                        <li>Thermal comfort: Do you feel comfortable right now? (yes or no)</li>
                                        <li>Thermal preference: Right now, would you prefer to be cooler, the same, or warmer?</li>
                                        <li>Access to personal controls (windows, fans, heaters, curtains, thermostat, doors)</li>
                              ...
                              <li>Physical measurements</li>
                                    <ul>
                                        ...
                                        <li>Outdoor temperature: Prevailing mean (monthly mean), Daily mean</li>
                 ...
                 p("Candido C. M. (2010) Indoor air movement acceptability and thermal comfort in hot-humid climates. PhD Thesis. Macquarie University, Australia."),
                 p("Candido C. M., de dear R., Lamberts R., Bittencourt L. S. (2010) Air movement acceptability limits and thermal comfort in Brazil hot humid climate zone. Building and Environment, 45, 222-229."),
                 ...
                 p("De Dear, R., Foldvary V., Zhang H., Arens E., Luo M., Parkinson T., Du X., Zhang W., Chun Ch., Liu S. Comfort in the mind of the beholder: A review of progress in adaptive thermal comfort research over the past two decades. The Fifth International Conference on Human-Environment System. ICHES 2016 Nagoya, October 29 - November 2, 2016."),
                 ...
                 p("Djamila H., Chu C., Kumaresan S. (2014) Effect of Humidity on Thermal Comfort in the Humid Tropics. Journal of Building Construction and Planning Research. 2, 109-117."),
                 p("Drake S., De Dear R., Alessi A., Deuble M. (2010) Occupant comfort in naturally ventilated and mixed-mode spaces within air-conditioned offices. Architectural Science Review. 53, 297-306."),
                 ...
                 p("Foldvary V. (2016) Assessment of indoor environmental quality in residential buildings before and after renovation. Doctoral thesis. Slovak University of Technology in Bratislava."),
                 ...
                 p("Kwok, A. G. and Chungyoon C. (2003) Thermal comfort in Japanese schools. Solar Energy 74(3), 245-252."),
                 ...
                 p("Pustayova Bukovianska H., Foldvary V., Petras D. (2015) Analysis of energy performance and indoor climate conditions of the Slovak housing stock before and after its renovation.  Energy Procedia 78, 2184-2189."),
                 ...
                 p("Singh M. K., Mahapatra S., Atreya S. K. (2011) Adaptive thermal comfort model for different climatic zones of North-East India, Applied Energy 88(7),  2420-2428."),
                 p("Singh M. K., Mahapatra S., Atreya S. K., Givi B. (2010) Thermal monitoring and indoor temperature modeling in vernacular buildings of North-East India, Energy and Buildings 42(10), 1610-1618."),
                 ...
                 p("Stoops, J. L. (2000) The thermal environmental occupant perceptions in European office buildings. Doctoral thesis. Chalmers University of Technology, 2000. ISBN: 993-356207-x."),
                 ...
                 p("This tool is originally established by", a("Margaret Pigman",href="https://www.linkedin.com/in/margaret-pigman-044a0024/"), "in 2014 for Database I, thereafter modified by ", a("Toby Cheung",href="https://www.researchgate.net/profile/Chin_To_Cheung"), "in 2016 implementing the survey of Database II and new features. 
                   Development of Database II is mainly contributed by the Centre for Built Environment (CBE) team, including ", a("Edward Arens",href="http://www.cbe.berkeley.edu/aboutus/staff-edward.htm"), ", ", a("Hui Zhang",href="http://www.cbe.berkeley.edu/aboutus/staff-hui.htm"), ", ", a("Veronika Foldvary",href="https://scholar.google.com/citations?user=xdHLdncAAAAJ&hl=en"), " and ", a("Maohui Luo",href="https://www.researchgate.net/profile/Maohui_Luo"), ",
                   ...
                   The ASHRAE Comfort Database Project II referred to in this description is partially funded by an American Society of Heating, Refrigerating and Air Conditioning Engineers grant in aid, Global Innovation Initiative, UK, Korea National Science Foundation, Center for the Built Environment, University of California at Berkeley, the Shakti Sustainable Energy Foundation and Ministry of New and Renewable Energy, Government of India, and the IHS BRE Press.
                   "),
                      br(),
                 p("We are also thankful to the data contributors, the list is presented here: "),
                 p("Alison Kwok, Andreas Wagner, Anoop Honnekeri, Antonio Augusto Xavier, Cheb Bouden, Christina Maria Candido, Chungyoon Chun, Cornelia Moosmann, Dennis Loveday, Harimi Djamila,
                    Dusan Petras, Fergus Nicol, Francesco Babich, Gail Brager, Gwelen Paliaga, Hana Pustayova Bukovianska, Hyojin Kim, Indraganti Madhavi, Jared Langevin, Jorn Toftum, Kazuyo Tsuzuki,
                    Kyle Konis, Linda Toledo, Lynda Webb, Malcolm Cook, Max Deuble, Michael Adebamowo, Michael Humphreys, Mila Mary Andamon, Nigel Oseland, Oluwafemi Akande, Paul Cropper, Rajan Rawal,
                    Ramona Romero, Renata De Vecchi, Salvatore Carlucci, Sanyogita Manu, Shanin Heidari, Shin-ichi Tanabe, Singh Manoj, Veronika Foldvary, Yingxin Zhu, Yongchao Zhai, Yufeng Zhang.
                   "),
                 
                 br(),                 


                      p("Contact Hui Zhang (zhanghui@berkeley.edu) or Veronika Foldary (veronika.foldvary@berkeley.edu) if you have data to contribute."),
                      br(),
